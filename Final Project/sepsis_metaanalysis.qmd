...
For binary variables, such as mortality among patients between the two groups, the `metabin` function from the [meta package](https://cran.r-project.org/web/packages/meta/meta.pdf) in R was utilized. The authors of the original review estimated a random effects model using a similar method, employing the Mantel-Haenszel approach.
...
![Forest plot from original study](sepsis_results_org.jpg)
